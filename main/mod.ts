...
export async function newScriptScript(sequenceNumber: number) {
  const privateKeyBytes = await Deno.readFile(privateKeyPath);
  const sender = DiemHelpers.hexToAccountAddress(senderAddress);
  ...
  const script = codegen.Stdlib.encodeNewScriptScript(testNftType);
  const payload = new DiemTypes.TransactionPayloadVariantScript(script);
  return await DiemHelpers.buildAndSubmitTransaction(
    fullSenderAddress,
    sequenceNumber,
